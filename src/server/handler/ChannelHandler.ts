...
export async function ChannelHandler(channel: string) {
    try {
        ...
        if (!Emotes?.emote_sets)
            throw `Failed to find ${targetUser} in 7TV Emotes`;
        ...
        if (doesChannelExist.rowCount > 0)
            throw `Channel ${targetUser} already exists in the database`;
        ...
        if (emotesListed.length === 0)
            throw `7TV returned no emotes for ${targetUser} (${data.id})`;
